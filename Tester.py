...
# Prüfe, ob die Datei existiert, um redundantes Laden zu vermeiden
data_file = "nasdaq.df"
...
if not os.path.exists(data_file):
    ...
    hist = pd.read_pickle(data_file)

# Berechnung des SMA 200 und prozentualer Abstand vom SMA 200
sma_days = 200
derivation = 3  # Prozent
hist['SMA'] = hist['Close'].rolling(window=sma_days, min_periods=1).mean()
...
for i in range(1, len(hist)-1):  # We reduce the range by 1 to avoid out-of-range errors
    ...
    if position_opened:
        ...
        investment_value_strategy_leveraged *= (1 + daily_return * leverage)
...
for i in range(1, len(hist)):
    ...
    buy_hold_values.append(buy_hold_values[-1] * (1 + daily_return))
    buy_hold_values_leveraged.append(buy_hold_values_leveraged[-1] * (1 + daily_return * leverage))
